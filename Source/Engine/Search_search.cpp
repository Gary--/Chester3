...
Search_SearchResult Search::search(const Search_Parameters p) {
	...
	if (Game::getRepeatCount()==1) {
		...
		if (ttRes.getDepth() >= p.depth) {
			if (ttRes.getType() == TT_Entry_Type::LOWER_BOUND && ttRes.score >= p.beta) {
				result.score = ttRes.score;
				result.pv.move = ttRes.getMove();
				return result;
			}

			if (ttRes.getType() == TT_Entry_Type::EXACT && p.beta == p.alpha+1) {
				result.score = ttRes.score;
				result.pv.move = ttRes.getMove();
				return result;
			}

			if (ttRes.getType() == TT_Entry_Type::UPPER_BOUND && ttRes.score <= p.alpha) {
				result.score = ttRes.score;
				return result;
			}
		}
